...
{
	internal class LaboratoryTrialProactiveInteractive : State
	{

		public LaboratoryTrialProactiveInteractive(App app) : base(app)
		{
		}
		...
		void LoadConfig()
		{
			...
			var config = new ProactiveInteractionConfig(new ConfigFile(path));

			_lateralOffsetMeter = config.LateralOffsetMeter;
			_probabilityMovementToRightPct = config.ProbabilityMovementToRightPct;

			Debug.Log($"[ProactiveInteractive] Loaded config from {path}: " +
			          $"{ProactiveInteractionConfig.LateralOffsetMeterKey}={_lateralOffsetMeter}, " +
			          $"{ProactiveInteractionConfig.ProbabilityMovementToRightPctKey}={_probabilityMovementToRightPct}");
		}
		...
		float _updateTimer;
		float _lateralOffsetMeter;
		int _probabilityMovementToRightPct;
	}
}
